...
void Day_Mode()
{
    ...
    // Step 1: Set NS to GREEN and all the other lights to RED
    Set_NS_LT(RED);
    Set_EW(RED);
    Set_EW_LT(RED);
    Set_NS(GREEN);
    
    if (NS_PED_SW == 1) // If the switch for NS is on, refer to the PED control 
    {
        PED_Control(NS, 8); // Sets it for the direction of NS and for 8 seconds
    }
    
    Wait_N_Seconds(7);  //Step 2: Wait 7 seconds with the NS light still on GREEN
    Set_NS(YELLOW); //Step 3: NS is set to YELLOW for 3 seconds
    Wait_N_Seconds(3);
    
    Set_NS(RED);    //Step 4: NS is set to RED
    
    if (EW_LT_SW == 1)   // Step 5: If EWLT switch is on, do the following steps
    {
        Set_EW_LT(GREEN);    // EWLT is set to GREEN for 8 seconds
        Wait_N_Seconds(8);
        Set_EW_LT(YELLOW); // It is then set to YELLOW for 3 seconds
        Wait_N_Seconds(3);
        Set_EW_LT(RED); // It is then set to RED
    }
    
    Set_EW(GREEN);  //Step 9: Set EW to GREEN
    
    if (EW_PED_SW == 1) // If EW switch is on, refer to PED control
    {
        PED_Control(EW, 9); // PED control set for EW direction for 9 seconds
    }
    
    Wait_N_Seconds(9);  //Step 10: EW is still GREEN for 9 seconds
    Set_EW(YELLOW); // After 9 seconds, EW is set to YELLOW
    Wait_N_Seconds(3);
    Set_EW(RED); // After 3 seconds, EW is set to RED
    
    if (NS_LT_SW ==1)    // Step 13: If NSLT switch is turned on do the following
    {
        Set_NS_LT(GREEN); // NSLT is set to GREEN for 8 seconds
        Wait_N_Seconds(8);
        Set_NS_LT(YELLOW); // After the 8 seconds, NSLT is set to YELLOW
        Wait_N_Seconds(3);
        Set_NS_LT(RED); // After 3 seconds, NSLT is set to RED
    }
}
void Night_Mode()
{ 
    ...
    Set_NS_LT(RED);  // Step 1: Set the NS light to GREEN while the others are set to RED
    Set_EW(RED);
    Set_EW_LT(RED);
    Set_NS(GREEN);
    
    Wait_N_Seconds(6);  // Step 2: Wait 6 seconds while NS is GREEN
    
    Set_NS(YELLOW);     // Step 3: After 6 seconds, NS turns yellow for 3 seconds
    Wait_N_Seconds(3);  
    
    Set_NS(RED);    // Step 4: After the 3 seconds, NS is now RED
    
    if (EW_LT_SW == 1)   // Step 5: If the switch is on, do the following steps
    {
        Set_EW_LT(GREEN);    // EWLT light is set to GREEN for 6 seconds
        Wait_N_Seconds(6);
        Set_EW_LT(YELLOW); // After 6 seconds, EWLT is set to YELLOW for 3 seconds
        Wait_N_Seconds(3);
        Set_EW_LT(RED); // After 3 seconds, EWLT is set to RED
    }
    
    Set_EW(GREEN);  //Step 9: EW is set to GREEN for 6 seconds
    Wait_N_Seconds(6);
    
    Set_EW(YELLOW); //Step 10: EW is set to YELLOW for 3 seconds
    Wait_N_Seconds(3);
    
    Set_EW(RED);    //Step 11: EW is set to RED
    
    if (NS_LT_SW == 1)   //Step 12: If the switch for NSLT is on, do the following steps
    {
        Set_NS_LT(GREEN); // Set the light to green for 8 seconds
        Wait_N_Seconds(8);
        Set_NS_LT(YELLOW);// After the 8 seconds, NSLT to set to YELLOW
        Wait_N_Seconds(3);
        Set_NS_LT(RED); // After 3 seconds, NSLT to set to RED
    }
